"""Compact single-run FM table (slice 104 — generalises compare column layout)."""
...
from rcm_desktop.adapter.faalwijze_analyse_service import FaalwijzeSingleRunPresentation
...
class FMSingleRunTableModel(QAbstractTableModel):
    def __init__(
        self,
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole):
        ...
        if col == "label":
            return row.label
        if col == "metric":
            return self._format_metric(row.metric_value)
        if col == "is_nmf":
