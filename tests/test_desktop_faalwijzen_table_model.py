...
from rcm_desktop.adapter.faalwijzen_grid_contract import SLICE_FIELD_KEYS
...
def test_set_data_nl_decimal_cost(sample_project):
    ...
    fm_row = next(r for r in svc_changed if r.row_key == "FM-001")
    assert fm_row.values["cost_cm_eur"] == pytest.approx(1.5)
...
def test_set_data_mttf_zero_shows_error_background_and_tooltip(sample_project):
    ...
    match = next(r for r in svc.rows() if r.row_key == "FM-001")
...
def test_fk_delegate_clear_and_set_roundtrip(sample_project):
    ...
    model.emit_grid_refresh()
    row_v = next(r for r in svc.rows() if r.row_key == "FM-001")
    assert row_v.values["functie_id"] == ""

    idx_known = editor.findData("FUNC-002")
    editor.setCurrentIndex(idx_known)
    delegate.setModelData(editor, model, ix)
    model.emit_grid_refresh()
    row_v2 = next(r for r in svc.rows() if r.row_key == "FM-001")
    assert row_v2.values["functie_id"] == "FUNC-002"
