...
def test_fm_toolbar_hides_view_toggle_outside_compare() -> None:
    from rcm_desktop.adapter.results_workspace_state import ResultsWorkspaceState
    ...
    assert profile is not None
    toolbar = _plan_output_fm_results_toolbar(snapshot, profile)
    assert toolbar.fm_compare_view_toggle_visible is False
...
def test_window_fm_compare_defaults_to_table_view(monkeypatch) -> None:
    ...
    assert window._fm_compare_view_mode == FM_COMPARE_VIEW_TABLE
...
def test_window_fm_compare_switch_to_diagram(monkeypatch) -> None:
    ...
    window._apply_fm_compare_panels(panels, snapshot, faalwijze_compare=pres)
    window.fm_compare_pane.setVisible(True)
    window.fm_compare_chart_scroll.resize(640, 480)
    window._set_fm_compare_view_mode(FM_COMPARE_VIEW_DIAGRAM)
    assert window._fm_compare_view_mode == FM_COMPARE_VIEW_DIAGRAM
