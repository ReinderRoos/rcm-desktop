...
def test_kpi_view_menu_shortcut_from_registry() -> None:
    from rcm_desktop.adapter.workspace_view_menu import workspace_view_menu_groups

    output_items = next(g for g in workspace_view_menu_groups() if g.group_id == "view_output").items
    kpi = next(i for i in output_items if i.view_id == "output.kpi_overview")
    assert kpi.shortcut == "Ctrl+K"
...
def test_workspace_menu_spec_is_valid_golden() -> None:
    ...
    assert by_id["whatif.toggle"].shortcut == "Ctrl+Shift+W"
    assert by_id["run.slot_a"].shortcut == "Ctrl+Shift+A"
    assert by_id["run.slot_b"].shortcut == "Ctrl+Shift+B"
    assert "run.scenario_cm" not in by_id
    assert "run.scenario_pm" not in by_id
